//
// Created by hewro on 2021/10/14.
//

#include "bits/stdc++.h"

#define NOT_CONNECTED INT_MAX / 2
class Solution {
public:
    int networkDelayTime(vector<vector<int>>& times, int n, int k) {
        vector<vector<int>> graph(n+1,vector<int>(n+1,NOT_CONNECTED));
        vector<int> dist(n+1,NOT_CONNECTED);
        vector<int> path(n+1);
        dist[k] = 0;
        dist[0] = 0;
        path[0]= -1;
        ...
        while(!T.empty()){
            ...
            for (int i = 1; i <= n; ++i) {
                ...
                if (i!=k && i!= min_i && graph[min_i][i] != NOT_CONNECTED){
                    if (dist[min_i] + graph[min_i][i] < dist[i]){
                        path[i] = min_i;
        ...
        return max == NOT_CONNECTED ?  -1 : max;
    }
};
...
int main(){
    ...
    std::cout << solution.networkDelayTime(times,15,8);
}